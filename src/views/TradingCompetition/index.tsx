...
const TradingCompetition = () => {
  ...
  const [currentPhase, setCurrentPhase] = useState(
    CompetitionPhases.REGISTRATION
  );
  ...
  const [userTradingInformation, setUserTradingInformation] =
    useState<UserTradingInformationProps>({
      hasRegistered: false,
  ...
  const [globalLeaderboardInformation, setGlobalLeaderboardInformation] =
    useState(null);
  ...
  // 1. Storm
  const [team1LeaderboardInformation, setTeam1LeaderboardInformation] =
    useState({ teamId: 1, leaderboardData: null });
  // 2. Flippers
  const [team2LeaderboardInformation, setTeam2LeaderboardInformation] =
    useState({ teamId: 2, leaderboardData: null });
  // 3. Cakers
  const [team3LeaderboardInformation, setTeam3LeaderboardInformation] =
    useState({ teamId: 3, leaderboardData: null });
  ...
  useEffect(() => {
    const fetchCompetitionInfoContract = async () => {
      const competitionStatus =
        await tradingCompetitionContract.currentStatus();
  ...
  useEffect(() => {
    ...
    const fetchTeamsLeaderboardStats = async (
      teamId: number,
      callBack: (data: any) => void
    ) => {
      try {
        const res = await fetch(
          `${profileApiUrl}/api/leaderboard/team/${teamId}`
        );
        const data = await res.json();
        callBack(data);
      } catch (e) {
  ...
  return (
    <CompetitionPage>
      ...
      <PageSection
        ...
        dividerComponent={
          <RibbonWithImage
            imageComponent={<PrizesIcon width="175px" />}
            ribbonDirection="up"
          >
            {t("Prizes")}
          </RibbonWithImage>
